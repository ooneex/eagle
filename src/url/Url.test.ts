...
describe('Url', () => {
  it('should parse a basic URL correctly', () => {
    ...
    expect(url.queries.get('query')).toBe('value');
  ...
  it('should parse multiple query parameters correctly', () => {
    ...
    expect(url.queries.get('name')).toBe('john');
    expect(url.queries.get('age')).toBe(25);
    expect(url.queries.get('active')).toBe(true);
  });
  ...
  it('should handle URLs without path, query or fragment', () => {
    ...
    expect(url.queries.count()).toBe(0);
